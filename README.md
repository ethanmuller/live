...
This runs the Vite dev server (frontend) and the Express/Socket.IO backend together. The frontend listens on port `8008` by default and proxies `/api` and `/socket.io` requests to the backend on port `8009`, so the whole app behaves as a single origin at `http://localhost:8008`. Both ports are configurable — see [Configuration](#configuration) below.
...
- `PORT` — the Express/Socket.IO backend's port. Also what Vite proxies `/api`/`/socket.io` to in dev.
